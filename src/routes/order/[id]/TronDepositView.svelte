<script lang="ts">
	...
	function updateCountdown() {
		if (!order.tronDeposit) return;

		const now = Date.now();
		const remaining = order.tronDeposit.expiresAt - now;
	...
	async function copyAddress() {
		...
		await navigator.clipboard.writeText(order.tronDeposit.depositAddress);
		copied = true;
	...
	const expectedAmount = $derived(
		order.tronDeposit
			? formatAtomicToDecimal(order.tronDeposit.expectedAmount, 6, {
					maxFractionDigits: 6,
					useGrouping: true
				})
			: '0'
	...
	<div class="mt-6 space-y-4" in:fade={{ duration: 150 }}>
		...
		<div class="rounded-xl border border-zinc-200 p-4 dark:border-zinc-700">
			...
			<div class="flex items-center gap-2">
				...
				>
					{order.tronDeposit.depositAddress}
				</code>
				<button
					onclick={copyAddress}
