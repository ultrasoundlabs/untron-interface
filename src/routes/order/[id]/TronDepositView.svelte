<script lang="ts">
	...
	import type { TronToEvmOrderView } from '$lib/types/swap';
	import { formatAtomicToDecimal } from '$lib/math/amounts';

	interface Props {
		order: TronToEvmOrderView;
	}
	...
	const expectedAmount = $derived(
		formatAtomicToDecimal(order.expectedAmount, 6, {
			maxFractionDigits: 6,
			useGrouping: true
		})
	);
...
<div class="mt-6 space-y-4" in:fade={{ duration: 150 }}>
	...
	<div class="rounded-xl border border-zinc-200 p-4 dark:border-zinc-700">
		...
		<div class="mx-auto mb-4 w-fit rounded-xl bg-white p-3 dark:bg-zinc-800">
			...
			>
				<QRCode data={order.depositAddress} size={160} errorCorrectionLevel="M" />
			</div>
		</div>
